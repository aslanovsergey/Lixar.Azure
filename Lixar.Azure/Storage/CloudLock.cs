...
namespace Lixar.Azure.Storage
{
    ...
    public class CloudLock : IDisposable
    {
        ...
        public async Task<bool> TryAcquireAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                LeaseId = await Blob.AcquireLeaseAsync(AcquireTimeSpan, null, cancellationToken).ConfigureAwait(false);
                ...
                {
                    await Blob.RenewLeaseAsync(AccessCondition.GenerateLeaseCondition(LeaseId), renewCancellationTokenSource.Token);
